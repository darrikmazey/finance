...
#   require File.join(File.dirname(__FILE__), '../lib/custom_fields')
#
# Example: 
#
#   -- view code
...
#     <% custom_fields_for(object, { :builder => MyFieldBuilder }) do |f| %>
#
# - create MyFieldBuilder class
#   class MyFieldBuilder << Dmt::CustomFields::CustomFieldsView
#     def field(name, value)
#       <<output your html here>>
#     end
#     
#     def before_fields_for(obj, options)
#       <<header code placed before the fields>>
#     end
# 
#     def after_fields_for(obj, options)
#       <<footer code placed after the fields>>
#     end
#   end
  ...
  module CustomFields
    module Helper
      def custom_fields_for(obj, options = {})
        options[:builder] = Dmt::CustomFields::CustomFieldsView.new unless options[:builder]
        options[:builder] = options[:builder].new if options[:builder].class == Class
        concat(options[:builder].before_fields_for(obj, options))
        yield Dmt::CustomFields::CustomFields.new(obj, options)
        concat(options[:builder].after_fields_for(obj, options));
      end
    end 
...
# inject the helper function into ActionView::Base
ActionView::Base.send :include, Dmt::CustomFields::Helper
